...
* Be aware that the `union` and `join` stages do not preserve sort order.
If you require a specific sort order after combining datasets, always place the `sort` stage **after** your `union` or `join` stage.
...
* Sort operations can concentrate data on a single worker, which for large datasets might exceed its capacity.
Including a `limit` stage after a `sort` stage can help reduce the maximum memory a worker needs to devote to the sort task, thereby improving performance.
