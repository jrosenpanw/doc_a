...
Use the `json_extract_array()` function to accept a string that represents a JSON array and returns an XQL-native array.
This function is crucial for transforming JSON-formatted array data into a format that XQL can natively process for further analysis.
