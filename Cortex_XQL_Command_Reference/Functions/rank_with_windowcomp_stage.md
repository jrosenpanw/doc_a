...
* **Difference from row_number**: Unlike `row_number()`, which always assigns unique sequential numbers, `rank()` assigns the same number to tied rows and leaves gaps.
* **Difference from dense_rank**: Unlike `dense_rank()`, which does not leave gaps after ties, `rank()` leaves gaps (for example, 1, 2, 2, 4 instead of 1, 2, 2, 3).
